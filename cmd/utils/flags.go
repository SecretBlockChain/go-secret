...
import (
	...
	"github.com/SecretBlockChain/go-secret/accounts/keystore"
	...
	"github.com/SecretBlockChain/go-secret/eth"
	"github.com/SecretBlockChain/go-secret/eth/downloader"
	"github.com/SecretBlockChain/go-secret/eth/gasprice"
	"github.com/SecretBlockChain/go-secret/ethdb"
	"github.com/SecretBlockChain/go-secret/ethstats"
	"github.com/SecretBlockChain/go-secret/graphql"
	"github.com/SecretBlockChain/go-secret/internal/ethapi"
	"github.com/SecretBlockChain/go-secret/internal/flags"
	"github.com/SecretBlockChain/go-secret/les"
	"github.com/SecretBlockChain/go-secret/log"
	"github.com/SecretBlockChain/go-secret/metrics"
	"github.com/SecretBlockChain/go-secret/metrics/exp"
	"github.com/SecretBlockChain/go-secret/metrics/influxdb"
	"github.com/SecretBlockChain/go-secret/miner"
	"github.com/SecretBlockChain/go-secret/node"
	"github.com/SecretBlockChain/go-secret/p2p"
	"github.com/SecretBlockChain/go-secret/p2p/discv5"
	"github.com/SecretBlockChain/go-secret/p2p/enode"
	"github.com/SecretBlockChain/go-secret/p2p/nat"
	"github.com/SecretBlockChain/go-secret/p2p/netutil"
...
var (
	...
	LocalnetFlag = cli.BoolFlag{
		Name:  "localnet",
		Usage: "Local network: pre-configured proof-of-equality test network",
	}
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
		if ctx.GlobalBool(LegacyTestnetFlag.Name) || ctx.GlobalBool(LocalnetFlag.Name) {
			// Maintain compatibility with older Geth configurations storing the
			// Localnet database in `testnet` instead of `localnet`.
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.GlobalBool(LegacyTestnetFlag.Name) || ctx.GlobalBool(LocalnetFlag.Name):
		urls = params.LocalnetBootnodes
...
func setBootstrapNodesV5(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.GlobalBool(LocalnetFlag.Name):
		urls = params.LocalnetBootnodes
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "SIE", ethPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
func setDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case (ctx.GlobalBool(LegacyTestnetFlag.Name) || ctx.GlobalBool(LocalnetFlag.Name)) && cfg.DataDir == node.DefaultDataDir():
		// Maintain compatibility with older Geth configurations storing the
		// Localnet database in `testnet` instead of `localnet`.
		legacyPath := filepath.Join(node.DefaultDataDir(), "testnet")
		if _, err := os.Stat(legacyPath); !os.IsNotExist(err) {
			log.Warn("Using the deprecated `testnet` datadir. Future versions will store the Localnet chain in `localnet`.")
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	CheckExclusive(ctx, DeveloperFlag, LegacyTestnetFlag, LocalnetFlag)
	...
	case ctx.GlobalBool(LegacyTestnetFlag.Name) || ctx.GlobalBool(LocalnetFlag.Name):
		...
		cfg.Genesis = core.DefaultLocalnetGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.LocalnetGenesisHash)
	case ctx.GlobalBool(DeveloperFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
		}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	switch {
	case ctx.GlobalBool(LegacyTestnetFlag.Name) || ctx.GlobalBool(LocalnetFlag.Name):
		genesis = core.DefaultLocalnetGenesisBlock()
	case ctx.GlobalBool(DeveloperFlag.Name):
